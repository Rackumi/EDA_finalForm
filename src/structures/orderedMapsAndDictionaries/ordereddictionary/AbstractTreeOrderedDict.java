...
abstract public class AbstractTreeOrderedDict<K, V> implements OrderedDictionary<K, V> {
     ...
     */
    private class DictEntry<T, U> implements Entry<T, U>, Comparable<Entry<T, U>> {
    ...
    public Iterable<Entry<K, V>> findAll(K key) {
        ...
        Iterable<? extends Position<Entry<K, V>>> posList = this.bsTree.findAll(entry);
    ...
    public Iterable<Entry<K, V>> findRange(K minkey, K maxkey) {
        DictEntry<K, V> min = new DictEntry<>(minkey, null, null, keyComparator);
        DictEntry<K, V> max = new DictEntry<>(maxkey, null, null, keyComparator);
        Iterable<Position<Entry<K, V>>> x = this.bsTree.findRange(min, max);
    ...
    public Entry<K, V> succesor(Position<Entry<K, V>> ent){
        return this.bsTree.successor(ent).getElement();
    }

    public Entry<K, V> predecessor(Position<Entry<K, V>> ent){
        return this.bsTree.predecessor(ent).getElement();
    }

    public Iterable<Entry<K, V>> succesors(Position<Entry<K, V>> ent){
        Iterable<Position<Entry<K, V>>> x = this.bsTree.successors(ent);
        List<Entry<K, V>> l = new LinkedList<>();
        for(Position<Entry<K,V>> p: x){
            l.add(p.getElement());
        }
    ...
    public Iterable<Entry<K, V>> predecessors(Position<Entry<K, V>> ent){
        Iterable<Position<Entry<K, V>>> x = this.bsTree.predecessors(ent);
        List<Entry<K, V>> l = new LinkedList<>();
        for(Position<Entry<K,V>> p: x){
            l.add(p.getElement());
        }
