...
//    }
    ...
    protected void remedyDoubleRed(RBInfo<E> nodeZ) {
        ...
        boolean blackUncle = false;
        if(uncleZ == null){
            blackUncle = true;
        }
        else if(!uncleZ.getElement().isRed){
            blackUncle = true;
        }
    ...
    public void remove(Position<E> posV) throws RuntimeException {
...
//		}
    ...
    @Override
    public int removeReturn(Position<E> posV) throws IllegalStateException {
        RBInfo<E> nodeV = this.checkPosition(posV); // may throw an InvalidPositionException
        boolean VisRed = nodeV.isRed();
        E toReturn = posV.getElement(); // entry to be returned
        //Witness to testify where nodeV originally was...
...
//		}
    ...
    public Iterable<Position<E>> findRange(E minValue, E maxValue) throws RuntimeException {
        ...
        List<Position<E>> salida = new ArrayList<>();
    ...
    public Position<E> first() throws RuntimeException {
        ...
        if(this.bst.binTree.root().getElement() == null){
            throw new RuntimeException("No first element.");
        }
        Position<E> salida = null;
        Iterator<Position<E>> it = this.iterator();
        while(it.hasNext()){
            salida = it.next();
            break;
    ...
    public Iterable<Position<E>> successors(Position<E> pos) {
        //TODO: Practica 5 Ejercicio 2
        LinkedList<Position<E>> salida = new LinkedList<>();
        structures.tree.binarySearchTree.DefaultComparator<E> comparator = new structures.tree.binarySearchTree.DefaultComparator<>();
        Iterator<Position<E>> it = this.iterator();
        while(it.hasNext()){
