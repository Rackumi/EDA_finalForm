package examenes.extraordinaria_2019_2020.ejercicio2;
...
public class MemoryMap<V> {

    int overwrite = 0;
    int failedRemoves = 0;


    public int countPutOverwrite() {
        return this.overwrite;
    }

    public int countFailedRemoves() {
        return this.failedRemoves;
    }

    public Iterable<?> lastQueries() {
        MemoryHashTableMap<?,?> m = new MemoryHashTableMap<>();
        return m.getLastQ();
    }

    private class MemoryHashTableMap<K, V> implements Map<K, V> {

        private Deque<V> lastQ = new LinkedList<>();

        public Deque<V> getLastQ() {
            return lastQ;
        }
         ...
         */
        protected MemoryHashTableMap(int cap) {
         ...
         * @param cap initial capacity
         */
        protected MemoryHashTableMap(int p, int cap) {
            this.n = 0;
            this.prime = p;
            this.capacity = cap;
        ...
        public V get(K key) throws IllegalStateException {
            ...
            lastQ.addFirst(bucket[i.index].getValue());
